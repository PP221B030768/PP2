...
class Snake: # создаем класс Snake
    ...
    def move(self): # функция по передвижению
        ...
        elif self.direction == 'UP': # если направление - "вверх" и змейка не выходит за границы экрана, то она двигается вниз (координаты элементов по Оу уменьшаются на 5), а в противном случае возвращается вниз, на конечную координату по Оу
...
class Food: # создаем класс Food
    def __init__(self): # функция для инициализации
        self.x = random.randint(21, 771) # центр окружности выбирается рандомно
